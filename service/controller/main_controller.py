...
from service.service.main_service import Equation
...
class MainService(Resource):
    ...
    def post(self) -> dict:
        try:
            ...
            main_service = Equation()
            resp = main_service.executar_rest(dados_request)
            response = objResponse.send_success(data=resp, messages=mensagens.SUCESSO_SOLUCAO, status=codeHttp.SUCCESS_200)
